...
from model.structures.trajectory_ops import * 
from datasets.JAAD_origin import JAAD
from . import transforms as T
from model.utils.dataset_utils import bbox_to_goal_map
import copy
import glob
import time
import pdb
...
class JAADDataset(data.Dataset):
    def __init__(self, cfg, split):
        ...
        # the reason of no crossing intention is that the birtap author manually set the intention to 0. already fix in JAAD_origin.py
    ...
    def __getitem__(self, index):
        ...
        if 'intent_label' in self.data:
            ret['intent_label'] = self.data['intent_label'][index]
        
        return ret

    def __len__(self):
        return len(self.data['obs_bbox'])
    ...
    def get_data(self, data, **model_opts):
        ...
        for k in data_tracks.keys():
            if k == 'intent':
                intent_label = np.array(data_tracks[k])[:,0].squeeze()
                continue
            obs_slices[k] = []
            pred_slices[k] = []
        ...
        if intent_label is not None:
            print('crossing prob: ', intent_label.sum()/intent_label.shape[0])
            ret['intent_label'] = intent_label
        return ret
