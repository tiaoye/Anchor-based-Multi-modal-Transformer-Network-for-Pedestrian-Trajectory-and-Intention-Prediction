...
from model.utils.box_utils import cxcywh_to_x1y1x2y2, x1y1x2y2_to_cxcywh
import torch
from tqdm import tqdm


from sklearn.metrics import roc_auc_score
def evaluate_intent(all_pred_intentions:np.ndarray, all_intent_labels:np.ndarray, threshold=0.5):
    '''
    evaluate intention prediction accuracy.
    all_pred_intentions: (B,2) float
    all_intent_labels: (B,) bool
    '''
    # calculate auc
    all_pred_intentions = all_pred_intentions[:, 1]
    auc = roc_auc_score(all_intent_labels, all_pred_intentions)
    # calculate accuracy
    all_pred_intentions = all_pred_intentions > threshold
    acc = (all_pred_intentions == all_intent_labels).mean()
    # calculate precision and recall
    tp = ((all_pred_intentions == 1) & (all_intent_labels == 1)).sum()
    fp = ((all_pred_intentions == 1) & (all_intent_labels == 0)).sum()
    fn = ((all_pred_intentions == 0) & (all_intent_labels == 1)).sum()
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)
    F1 = 2 * precision * recall / (precision + recall)
    ret = {'AUC': auc, 'Accuracy': acc, 'Precision': precision, 'Recall': recall, 'F1': F1, 'threshold': threshold}
    return ret
